...
public class SpawnstructurescaryhouseProcedure {
	@SubscribeEvent
		...
		if (Math.random() > 0.9999999) {
			if (world.getLevelData().getGameRules().getBoolean(RecollectionModGameRules.GRACEFUL) == false) {
				if (world instanceof ServerLevel _serverworld) {
					StructureTemplate template = _serverworld.getStructureManager().getOrCreate(ResourceLocation.fromNamespaceAndPath("recollection", "scaryhouse"));
					if (template != null) {
